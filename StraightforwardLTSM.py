...
LSTMModel.add(Embedding(max_features, 32))
...
TestScores = LSTMModel.predict(X_test)
TestScores2 = LSTMModel.predict_classes(X_test)
...
plt.plot(roc_curve(test_labels, TestScores, drop_intermediate=Trues ))
plt.show()

y_pred_rf = rf.predict_proba(X_test)[:, 1]
fpr_rf, tpr_rf, _ = roc_curve(test_labels, TestScores)

plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr_rf, tpr_rf, label='NN')
...
accuracy_score(test_labels, TestScores)
